...
通过完成这个项目，使得我对snort网络入侵检测系统有了一个具体的了解，Snort IDS(入侵检测系统)是一个强大的网络入侵检测系统。它具有实时数据流量分析和记录网络数据流包的能力。对目标网络从源IP源端口到目的IP和目的端口的整个过程进行整体监控，能够进行协议分析，对网络数据包内容进行搜索和匹配。它能够检测各种不同的网络攻击方式，对异常数据流量实时监控和及时警报。
